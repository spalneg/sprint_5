...
def test_exit_button_logout_by_click(email, password):
    ...
    browser.find_element(By.XPATH, '//button["Выход"]').click()
    WebDriverWait(browser, 5).until(expected_conditions.visibility_of_element_located((By.XPATH, '//button[text()="Войти"]')))
    assert browser.current_url == 'https://stellarburgers.education-services.ru/login'
    browser.quit()